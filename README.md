...
- Spring Boot JPA 프로젝트 개발 계속
	...
	3. /controller/BoardContriller.java 생성 후 /board/list url을 실행할 수 있는 메서드 작성
  ...
  12. /templates/board/list.html, detail.html Bootstrap 적용
      <img src="https://github.com/iieunji023/basic-springboot-2024/blob/main/images/sp004.png" width="650">
...
- Spring Boot JPA 프로젝트 개발 계속
    ...
    4. 스프링시큐리티(그다음 중요!)
      ...
      - (설정) Gradle 재빌드, 서버 실행
      - user / 로그상 UUID(서버 실행시마다 변경) 입력
        - 로그: Using generated security password: 565cb989-58b9-4de6-8755-6bf62e5d9a22
      - /security/SecurityConfig.java 보안설정 파일 생성, 작성
...
- Spring Boot JPA 프로젝트 개발 계속
	...
	2. 로그인 기능
		...
		- /service/MemberSecurity.java 로그인은 post를 사용하지않고, Spring Security가 지원하는 UserDetailsService 인터페이스 활용
	...
	3. 게시글 작성자 추가
		...
		- /config/SecurityConfig.java에 @PreAuthorize 동작하도록 설정 추가
...
- Spring Boot JPA 프로젝트 개발 계속
	1. 수정, 삭제 기능
		...
		- /templates/board/create.html form 태그에 있는 th:action 삭제
			...
			- get이 /board/create로 들어가면 post도 같은 URL로 실행되고, /board/modify/{bno}로 들어가면 post도 같은 URL로 실행
	...
	2. 앵커 기능
		...
		= /service/ReplyService.java setReply() 메서드 void -> Reply 변경
...
- Spring Boot JPA 프로젝트 개발 계속
	...
	2. 마크다운 적용
		...
		<img src="https://github.com/iieunji023/basic-springboot-2024/blob/main/images/sp009.png" width="730">
		
		<img src="https://github.com/iieunji023/basic-springboot-2024/blob/main/images/sp010.png" width="730">
...
- Spring Boot JPA 프로젝트 개발 계속
	1. 에러페이지 작업(404, 500, etc)
		...
		- resource/templates/error/404.html, 500.html, error.html 생성
	...
	2. 비밀번호 찾기, 비밀번호 변경
		...
		- 네이버 메일 SMTP 설정 > 환경설정 > POP3/IMAP 설정
			...
			<img src="https://github.com/iieunji023/basic-springboot-2024/blob/main/images/sp012.png" width="730">
...
- Spring Boot JPA 프로젝트 개발 계속
    ...
    1. 비밀번호 초기화(계속)
        ...
        - /service/MailService.java에 메일전송 메서드 생성, 수정
            ...
            <img src="https://github.com/iieunji023/basic-springboot-2024/blob/main/images/sp013.png" width="730">

        - /entity/Reset.java 생성
        - /repository/ResetRepository.java 인터페이스 생성, findByUuid() 추가
        - /service/ResetService.java 생성
        - /service/MailService.java에 REesetService 객체 생성, 메일전송 후 setReset() 사용
        ...
        - /service/MemberService.java에 setMember() 메서드 추가
          ...
          <img src="https://github.com/iieunji023/basic-springboot-2024/blob/main/images/sp014.png" width="730">
        
        - 이메일 화면 수정
        - /member/reset-password Get 메서드 작성
        - /html ....
...
- 리액트 개발환경
	1. node.js 설치
		- https://nodejs.org/en, Download Node.js(LTS) 설치
...
- 리액트 기본구조 및 개발 방법
	...
	2. 깃헙에 .gitignore 먼저 commit하고 push
...
- 리액트 기초공부
	...
	6. 목록 표시
		...
		- 각 child 요소마다 key 속성이 필요(없으면 경고!)
			...
			<img src="https://github.com/iieunji023/basic-springboot-2024/blob/main/images/react001.png" width="730">
			
	7. 이벤트 핸들링		
		- form + onSubmit, tag + onClick
		- 이벤트 파라미터 전달
		- 파라미터가 필요해서 함수 뒤에 ()를 쓰면 이벤트 순서에 따라 리플레시 후 자동 실행됨
	...
	8. 컴포넌트 간 데이터 전달
        ...
        - props.속성이름.key 이름
			...
			<img src="https://github.com/iieunji023/basic-springboot-2024/blob/main/images/react002.png" width="730">
...
- 리액트 추가내용
	...
	3. VS Code 확장
		...
		- Import Cost: 라이브러리 비용 계산해줌
		- VSCode React Refactor: 리팩팅 도구
		
	4. 리액트 디버거
		- 크롬, 엣지 브라우저별로 따로 존재
		- Reaact Developer Tools 설치
...
4. frontBoard 개발 시작
	...
	- Header.js에 react-router-dom 추가. Link, useNavigate 함수 사용
...
5. backboard RestAPI 추가
	...
	- (문제)Spring Boot에서 만든 Entity는 Board와 Reply 등의 OneToMany / ManyToOne가 JSON으로 변환할 때 문제 발생!
...
6. frontboard 개발 계속
	...
	- 테이블 내용을 boardList.map() 10개 리스트 디스플레이
		...
		<img src="https://github.com/iieunji023/basic-springboot-2024/blob/main/images/react003.png" width="730">
...
- Spring Boot React연동 프로젝트 개발 계속
	...
	2. 페이징
		...
		- /common/CommonFunc.js 생성 - 작성일 수정하기 위한 함수 formatDate() 작성
...
1. 로그인
	...
	- Login.js 화면 수정
	- (BackEnd) restcontroller/RestMemberController.java 생성, login() POST 메서드 작성
	- service/MemberService.java getMemberByUsernameandPassword() 메서드 추가
	- Postman에서 테스트
